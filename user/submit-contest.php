...
// Get attempt details
$stmt = $pdo->prepare("
    SELECT ua.*, c.duration_minutes, 
           TIMESTAMPDIFF(SECOND, ua.start_time, NOW()) AS time_taken
...
if (!$attempt) {
    header("Location: user-contest.php?error=Invalid attempt");
    exit();
}

// Check if time limit exceeded
if ($attempt['duration_minutes'] > 0) {
    $maxSeconds = $attempt['duration_minutes'] * 60;
    if ($attempt['time_taken'] > $maxSeconds) {
...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE id = ?
    ");
    $stmt->execute([$score, $timeTaken, $attemptId]);
    ...
    $pdo->rollBack();
    header("Location: user-contest.php?error=Failed to submit answers");
    exit();
}

header("Location: contest-results.php?attempt_id=" . $attemptId);
exit();